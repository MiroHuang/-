//
//  StrongBoxTurnBackViewController.m
//  新濠城
//
...
@interface StrongBoxTurnBackViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation StrongBoxTurnBackViewController
...
- (void)_initializedData {
    ...
    _leftViewFromTextField_Width = [NSString checkSizeFromString:@"身上分" Font:_leftViewFromTextField_Font Width:MAXFLOAT Height:MAXFLOAT].width;
}
- (void)_initTableView {
    ...
    [self.kCompleteButton bk_addEventHandler:^(id sender) {
        ...
        if ([KUserInfoDataModel shareInstance].kUserInfoModel.ispaypassword) {
            KPayAlertViewController *alertController = [KPayAlertViewController showQyjCustomerAlertViewController:^(NSString *resultVault) {
                kStrongfy(kWeakSelf);
                [kStrongSelf sendRequestForStrongBoxTurnBack:resultVault];
            }];
...
#pragma mark - Request
- (void)sendRequestForStrongBoxTurnBack:(NSString *)password {
    NSMutableDictionary *parameters = [NSMutableDictionary dictionary];
    [parameters setValue:@"tosafebalance" forKey:@"action"];
...
- (UIButton *)kCompleteButton {
    if (!_kCompleteButton) {
        ...
        [_kCompleteButton setTitle:@"确定转到保险箱" forState:UIControlStateNormal];
